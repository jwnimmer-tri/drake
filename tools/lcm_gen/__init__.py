...
@@GET_ENCODED_SIZE@@
  ...
  // XXX Add an encode-to-inout-string or encode-to-inout-vector?
...
class CppGen:
    ...
    def _fill_one_encoded_size_dynamic(self, field):
        ...
        if field.typ == PrimitiveType.string:
            lines.append(f"{pad}_result += sizeof(int32_t) + {var}.size() + 1;")  # noqa
        else:
    ...
    class Ingredient(NamedTuple):
        # Encoding the message will be accomplished through a sequence of
        # operations, each of which returns true on success (or false on
        # failure). Some operations also have a pre-condition that the cursor
        # have enough bytes remaining which will need to be checked before
        # running the operation.
        operations: List[str]
        required_bytes: Optional[str] = None
        fields: Optional[List[str]] = None
        dim_fields: Optional[List[str]] = None

    def _stir(self, a: Ingredient, b: Ingredient):
        # When adjacent ingredients in the recipe all have a pre-condition on
        # the number of bytes required for their operations, we can hoist and
        # fuse their pre-condition checks into a single larger condition, which
        # is better for runtime encoding efficiency.
        if a.required_bytes is None:
            return None
        if b.required_bytes is None:
            return None
        for name in b.dim_fields or []:
            if any([x == name for x in a.fields or []]):
                return None
        return CppGen.Ingredient(
            operations=a.operations + b.operations,
            required_bytes=f"{a.required_bytes} + {b.required_bytes}",
            fields=(a.fields or []) + (b.fields or []),
            dim_fields=(a.dim_fields or []) + (b.dim_fields or []),
        )

    def _bake(self, recipe: List[Ingredient]):
        recipe = copy.copy(recipe)
        i = 0
        while i + 1 < len(recipe):
            a, b = recipe[i:i+2]
            stirred = self._stir(a, b)
            if stirred is not None:
                recipe[i:i+2] = [stirred]
            else:
                i += 1

        # Now we'll interleave all of the size checks and operations.
        all_operations = []
        for ingredient in recipe:
            if ingredient.required_bytes is not None:
                all_operations.extend([
                    f"(*_cursor + {ingredient.required_bytes} <= _end)",
                ])
            all_operations.extend(ingredient.operations)
    ...
    def _fill_one_encode(self, field):
        ...
        base = self._full_typename(field.typ)
        ndims = len(field.array_dims)
        return f"_encode_field<{base}, {ndims}>({field.name}, _cursor);"

    def _fill_decode(self):
        """Updates the decode() implementation for this message."""
        # The recipe for encoding the message involves various bounds checks
        # intermingled with the data encoding.
        recipe = [
            # Decode the hash and check it.
            CppGen.Ingredient(
                required_bytes="_hash_size",
                operations=[
                    "(with_hash ? _decode_field<int64_t, 0>(&_hash, _cursor, _end) : true)",  # noqa
            ...
            CppGen.Ingredient(
                fields=[field.name],
                required_bytes=self._known_encoded_size(field),
                dim_fields=[
                    x for x in field.array_dims
                    if isinstance(x, str)
                ],
                operations=self._fill_one_decode(field),
            )
            for field in self._struct.fields
        ]
    ...
    def _fill_one_decode(self, field):
        ...
        ndims = len(field.array_dims)
        operations = [
            f"_decode_field<{base}, {ndims}>("
            + f"&{field.name}, _cursor, _end{extra_array_dims})",
        ]
        if field.name in self._size_variables:
