...
def _maybe_setup_pkg_config_repository(repository_ctx):
    ...
    # Convert the canonical name (e.g., "+_repo_rules+pkgconfig_eigen") to its
    # apparent name (e.g., "pkgconfig_eigen") so that when a BUILD file uses a
    # label which omits the target name (e.g., deps = ["@pkgconfig_eigen"]) the
    # unabbreviated label (e.g., "@pkgconfig_eigen//:pkgconfig_eigen") will
    # match what we declare.
...
)""".format(name = repr(library_name), error = repr(error)))
