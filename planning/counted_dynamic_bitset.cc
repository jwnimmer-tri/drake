...
// Our inline storage uses uint64_t.
...
CountedDynamicBitset& CountedDynamicBitset::operator=(
  ...
  if (other.is_inline()) [[likely]] {
    ...
    buffer_ = MakeBoolArray(std::span<const bool>(other.buffer_.get(), other.size_));
  }
  return *this;
}
...
CountedDynamicBitset CountedDynamicBitset::Complement() const {
  if (is_inline()) [[likely]] {
    ...
    CountedDynamicBitset result(
        ...
        /* count = */ size() - count(),
        /* inline = */ 0);
    result.buffer_ = MakeComplement(std::span<const bool>(buffer_.get(), size()));
    return result;
  }
}
...
CountedDynamicBitset CountedDynamicBitset::Union(
  ...
  if (is_inline()) [[likely]] {
    const uint64_t new_bits = (this->inline_ | other.inline_) & mask();
