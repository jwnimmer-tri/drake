...
class RenderEngine {
 public:
  ...
  /** Clones the render engine.
   ...
   @tparam SmartPointer must be either std::unique_ptr or std::shared_ptr.
   In C++, it defaults to unique_ptr; in Python, it's hard-coded to shared_ptr.

   @throws std::exception if SmartPointer is std::unique_ptr but this particular
   class only supports cloning for std::shared_ptr (e.g., for engines written in
   Python). */
  template <template <typename> class SmartPointer = std::unique_ptr>
  SmartPointer<RenderEngine> Clone() const;
