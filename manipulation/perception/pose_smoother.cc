...
struct InternalState {
  ...
  RigidTransformd pose;  // Initializes to identity RigidTransform.
 ...
 * delta_t seconds.
 */
VectorX<double> ComputeVelocities(const RigidTransformd& pose_1,
                                  const RigidTransformd& pose_2,
                                  double delta_t) {
...
RigidTransformd PoseVector7ToRigidTransform(
  ...
  return RigidTransformd(quaternion, pose_vector.head<3>());
}

// Convert a pose into a 7 element array whose first 3 elements are position and
// last 4 elements are a quaternion (w, x, y, z) with w >= 0 (canonical form).
VectorX<double> RigidTransformdToVector7(const RigidTransformd& pose) {
...
PoseSmoother::PoseSmoother(double desired_max_linear_velocity,
    ...
    : max_linear_velocity_(desired_max_linear_velocity),
      ...
      use_eigen_isometry3_(use_eigen_isometry3) {
  if (use_eigen_isometry3_) {
    static const logging::Warn log_once(
        "PoseSmoother use of Isometry3 as input and output is deprecated and"
        " will be removed from Drake on or after 2021-11-01.");
    smoothed_pose_output_port_ = this->DeclareAbstractOutputPort(
        systems::kUseDefaultName,
        &PoseSmoother::OutputSmoothedPoseDeprecated).get_index();
  } else {
    smoothed_pose_output_port_ = this->DeclareAbstractOutputPort(
        systems::kUseDefaultName,
        &PoseSmoother::OutputSmoothedPose).get_index();
  }
  smoothed_velocity_output_port_ = this->DeclareAbstractOutputPort(
      systems::kUseDefaultName,
      &PoseSmoother::OutputSmoothedVelocity).get_index();
  ...
  if (use_eigen_isometry3_) {
    ...
    this->DeclareAbstractInputPort(
        systems::kUseDefaultName,
        Value<RigidTransformd>(RigidTransformd::Identity()));
  }
...
void PoseSmoother::DoCalcUnrestrictedUpdate(
  ...
  RigidTransformd input_pose;
  if (use_eigen_isometry3_) {
    input_pose = RigidTransformd(
        this->get_input_port(0).Eval<Isometry3d>(context));
  } else {
    input_pose = this->get_input_port(0).Eval<RigidTransformd>(context);
  }
  ...
  RigidTransformd& current_pose = internal_state.pose;
  ...
  if (accept_data_point) {
    RigidTransformd accepted_pose = input_pose;
...
void PoseSmoother::OutputSmoothedPoseDeprecated(
    const systems::Context<double>& context, Isometry3d* output) const {
...
void PoseSmoother::OutputSmoothedPose(
    const systems::Context<double>& context, RigidTransformd* output) const {
  const auto internal_state = context.get_abstract_state<InternalState>(0);
