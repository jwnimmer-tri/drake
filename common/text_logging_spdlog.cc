...
logger::logger() : impl_{get_spdlog_logger_singleton()} {}

logger::~logger() = default;

bool logger::should_log(level_enum severity) const {
  return static_cast<const spdlog::logger*>(impl_)->should_log(
      FromDrakeLevel(severity));
}

level_enum logger::level() const {
  return ToDrakeLevel(static_cast<const spdlog::logger*>(impl_)->level());
}

void logger::set_level(level_enum severity) const {
  return static_cast<spdlog::logger*>(impl_)->set_level(
      FromDrakeLevel(severity));
}

void logger::set_pattern(std::string_view pattern) {
  static_cast<spdlog::logger*>(impl_)->set_pattern(std::string{pattern});
}

void logger::LogUnconditionally(level_enum severity, std::string_view message) {
  return static_cast<spdlog::logger*>(impl_)->log(FromDrakeLevel(severity),
                                                  message);
}

sink* get_dist_sink() {
  // Extract the dist_sink_mt from Drake's logger instance.
  spdlog::logger* spdlog_logger = get_spdlog_logger_singleton();
  auto* sink = spdlog_logger->sinks().empty()
                   ? nullptr
