...
int do_main(int argc, char* argv[]) {
    ...
    const auto& cam_port =
        image_to_lcm_image_array
            .DeclareImageInputPort<systems::sensors::PixelType::kRgba8U>(
                "camera_" + name);
    builder.Connect(station["camera_" + name + "_rgb_image"], cam_port);
  }
  ...
  builder.Connect(image_to_lcm_image_array, image_array_lcm_publisher);
  ...
  station["iiwa_position"].FixValue(&station_context, q0);

  // Zero feed-forward torque.
  station["iiwa_feedforward_torque"]
      .FixValue(&station_context, VectorXd::Zero(station.num_iiwa_joints()));

  // Nominal WSG position is open.
  station["wsg_position"].FixValue(&station_context, 0.1);
  // Force limit at 40N.
  station["wsg_force_limit"].FixValue(&station_context, 40.0);
