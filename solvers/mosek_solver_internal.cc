...
void MosekSolverProgram::UpdateOptions(
  ...
  // Copy all options into our `task_`, except for "writedata" which needs
  // special handling.
  options->Respell([&](const auto& common) {
    ...
    *is_printing = false;
    ...
    // None of the common options get converted into new option names.
    return string_unordered_map<SolverOptions::OptionValue>{};
  });
