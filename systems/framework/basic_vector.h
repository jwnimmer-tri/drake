...
%BasicVector cannot be copied or moved; use Clone instead.  (We cannot support
copy or move because of the slicing problem, and also because assignment of a
BasicVector could change its size.)
...
class BasicVector : public VectorBase<T> {
 public:
  ...
  placewise-corresponding member of @p args as the sole constructor argument.
  ...
  /** Sets the vector to the given value. After `a.set_value(b.get_value())`,
 ...
 protected:
  ...
  /** Returns a new BasicVector with the same size and type as this.  Callers
  must take ownership, and may rely on the NVI wrapper to initialize the clone
  elementwise. Subclasses of BasicVector must override DoClone to return their
  covariant type. */
