...
class DRAKE_NO_EXPORT BusValue::Impl {
 public:
  ...
  void IterNext(BusValue::Iterator* iterator) const;
...
void BusValue::Impl::IterNext(BusValue::Iterator* iterator) const {
...
BusValue::BusValue() : impl_(std::make_unique<Impl>()) {}
...
BusValue::Iterator& BusValue::Iterator::operator++() {
  DRAKE_THROW_UNLESS(impl_ != nullptr);
  static_cast<const BusValue::Impl*>(impl_)->IterNext(this);
  return *this;
}
...
BusValue::Impl& BusValue::impl() {
  return *impl_;
}

const BusValue::Impl& BusValue::impl() const {
  return *impl_;
}
