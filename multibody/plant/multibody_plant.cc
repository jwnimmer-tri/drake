...
void MultibodyPlant<T>::DeclareCacheEntries() {
  ...
  if (!is_discrete() && use_hydroelastic) {
    ...
    // EvalContactSurfaces
    const auto& contact_summary = get_geometry_query_input_port();
    tickets.insert(this->input_port_ticket(contact_summary.get_index()));
...
const SceneGraphInspector<T>& MultibodyPlant<T>::EvalSceneGraphInspector(
    const systems::Context<T>& context) const {
  // TODO(jwnimmer-tri) The "geometry_query" input port is invalidated anytime
  // kinematics changes, but really we only need invalidation of the inspector
  // when the scene graph topology or properties change. If we find that this
  // is a performance bottleneck, this is something we could clean up.
  const auto& query_object = EvalGeometryInputPort<geometry::QueryObject<T>>(
