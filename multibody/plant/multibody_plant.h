...
class MultibodyPlant : public internal::MultibodyTreeSystem<T> {
 public:
  ...
  /// Returns the inspector for the SceneGraph associated with this plant, via
  /// this plant's "geometry_query" input port. (In the future, the inspector
  /// might come from a different context source with less churn.)
  const geometry::SceneGraphInspector<T>& EvalSceneGraphInspector(
      const systems::Context<T>& context) const;
