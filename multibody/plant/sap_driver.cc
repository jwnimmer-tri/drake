...
namespace {
/* Returns true iff the given actuator has a PD controller and the controller is
enabled (its joint is not locked and it has at least one non-zero gain). */
bool IsPdControllerConstraintEnabled(const JointActuator<T>& actuator,
                                     const systems::Context<T>& context) {
  const auto& plant = actuator.GetParentPlant();
  if (!actuator.has_controller()) {
    return false;
  }
  const PdControllerGains& gains = actuator.get_controller_gains();
  if (gains.p == 0 && gains.d == 0) {
    // The controller has no feedback, so we skip it. Note that this also means
    // the joint's effort limit will not be enforced -- it's up to the user to
    // saturate their feed forward actuation input port as needed.
    return false;
  }
...
void SapDriver<T>::AddPdControllerConstraints(
  ...
  for (JointActuatorIndex actuator_index : plant().GetJointActuatorIndices()) {
    ...
    if (!IsPdControllerConstraintEnabled(actuator)) {
      continue;
    }
    const Joint<T>& joint = actuator.joint();
...
void SapDriver<T>::CalcActuation(const systems::Context<T>& context,
  ...
  *actuation = manager().AssembleActuationInput(context);
  ...
  for (JointActuatorIndex actuator_index : plant().GetJointActuatorIndices()) {
    const JointActuator<T>& actuator =
        plant().get_joint_actuator(actuator_index);
    if (!IsPdControllerConstraintEnabled(actuator, context)) {
      // PD actuation is considered to be zero and only the input actuation is
      // reported.
      continue;
    }
    const Joint<T>& joint = actuator.joint();
    const SapConstraint<T>& c = sap_problem.get_constraint(constraint_index++);
    const int dof = joint.velocity_start();
