...
bool CenicIntegrator<T>::DoStep(const T& h) {
  ...
  // reuse the same linearization, but thatt's a bit challenging because it
  ...
  if (t0 == time_at_last_solve_) {
    ...
    // XXX BROKEN we need to set the feedback gains here, too.
  } else {
...
void CenicIntegrator<T>::LinearizeExternalSystem(
  ...
  // Resize and/or reset our outputs as necessary. In steady state this is
  // a no-op, but if the user calls FixInputPort thigns might shift a bit.
  const int nq = plant_.num_positions();
  const int nv = plant_.num_velocities();
  if (has_actuation_forces) {
    if (!actuation_feedback->has_value()) {
      actuation_feedback->emplace();
      actuation_feedback->value().resize(nv);
    }
  } else {
    *actuation_feedback = std::nullopt;
  }
  if (has_external_forces) {
    if (!external_feedback->has_value()) {
      external_feedback->emplace();
      external_feedback->value().resize(nv);
    }
  } else {
    *external_feedback = std::nullopt;
  }
  ...
  // Extract the feedback gains output vectors that we'll set.
  VectorX<T> empty;
  VectorX<T>& Ku = has_actuation_forces ? actuation_feedback->value().K : empty;
  VectorX<T>& bu = has_actuation_forces ? actuation_feedback->value().b : empty;
  VectorX<T>& Ke = has_external_forces ? external_feedback->value().K : empty;
  VectorX<T>& be = has_external_forces ? external_feedback->value().b : empty;
  if (has_actuation_forces) {
